...
export default function ChatInterface() {
  ...
  const sendMessage = async (query: string) => {
    ...
    try {
      ...
      // Build error message with details if it's an error
      let content = data.answer || data.error || "Unable to process your query.";
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: `⚠️ **Connection Error**\n\nCould not reach the server. Please check:\n1. The app is deployed correctly\n2. Database tables exist (visit /api/seed)\n3. Environment variables are set in Vercel`,
      }]);
    } finally {
      setLoading(false);
    }
